...
<?php
	...
	if(validate()){
		...
		$check3=$db->prepare('SELECT count(*) as counttotal FROM bookings WHERE  (date = ? && place = ? && accepted<>-1)');
		$data3=array($date,$place); //for below 'if' statement
...
<?php
	...
	if(validate()){
		...
		$check3=$db->prepare('SELECT count(*) as counttotal FROM bookings WHERE  (date = ? && place = ?)');
		$data3=array($date,$place); //for below 'if' statement
		$check3->execute($data3);
